...
describe("Reader -> ReaderStorage Test", async () => {
    let vault               : any,
        router              : any,
        pm                  : any,
        shortsTracker       : any,
    ...
    it("reader.parameters => name", async() => {
        expect(reader.address).to.not.be.eq(constants.AddressZero);
        expect(await reader.BASIS_POINTS_DIVISOR()).to.be.eq(10000);
        expect(await reader.POSITION_PROPS_LENGTH()).to.be.eq(9);
        expect(await reader.PRICE_PRECISION()).to.be.eq(parseUnits("1.0", 30));
        expect(await reader.ZKUSD_DECIMALS()).to.be.eq(18);
        expect(await reader.hasMaxGlobalShortSizes()).to.be.false;
    });
    it("reader.parameters => gov()", async () => {
        expect(await reader.gov()).to.be.eq(owner.address);
        await expect(reader.connect(user0).setGov(user0.address)).to.be.revertedWith(ErrorsV2.GOVERNABLE_FORBIDDEN); //!gov

        await reader.setGov(user0.address); //gov
        expect(await reader.gov()).to.be.eq(user0.address);
        await reader.connect(user0).setGov(owner.address);
        expect(await reader.gov()).to.be.eq(owner.address);
    });
    it("reader.func => setConfig", async () => {
        expect(await reader.hasMaxGlobalShortSizes()).to.be.false;

        await expect(reader.connect(user0).setConfig(true)).to.be.revertedWith(ErrorsV2.GOVERNABLE_FORBIDDEN); //!gov
        await reader.setConfig(true);
        expect(await reader.hasMaxGlobalShortSizes()).to.be.true;
        await reader.setConfig(false);
        expect(await reader.hasMaxGlobalShortSizes()).to.be.false;
    });
});
