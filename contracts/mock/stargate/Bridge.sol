...
contract Bridge is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    StgRouter public immutable router;
    ...
    constructor(address _layerZeroEndpoint, address _router) {
        ...
        router = StgRouter(_router);
    }

    //---------------------------------------------------------------------------
    // EXTERNAL functions

    function bytesToAddress(bytes memory addressBytes) public pure returns (address) {
        require(addressBytes.length >= 20, "Invalid addressBytes length"); // Ensure addressBytes has at least 20 bytes
